...
def clean_cell_data(data): #remove incomplete entries
    to_be_removed = list()
    for cell in data:
...
def clean_gps_data(data): #remove incomplete entries
    to_be_removed = list()
    for gps in data:
        try:
...
def parse_dep_muni_data(data): #convert list of department/municipalities into a dictionary
    loc_data = dict()
    for loc in data:
        loc_data[loc[1]] = loc[0]
    return loc_data

def change_gps_data(data): #convert the numerical values that represent departments and municipalities into their actual names
...
def get_state_data(data): #make a dictionary where the department/state is the key and the value is a list containing a list of cities, a list of latitudes, a list of longitudes, and a list of cell IDs
    states = dict()
...
def get_city_data(data): #make a dictionary where the city/municipality is the key and the value is a list of the state, a list of latitudes, a list of longitudes, and a list of cell IDs
    cities = dict()
...
def get_avg_coords(locations): #average the coordinates of whatever is inputed, guaranteed to work on city and state data, however, has not been tested on anything else
    avg_coords = dict()
...
def get_closest_location(coordinates, lat, lon): #get the approximate closest city/state/tower to an inputed latitude and longitude
    coord = [lat, lon]
    closest_location = ""
...
def distance_between_locations(locations, avg_coords): #generates a dictionary where the keys are locations and the values are lists that contain the name of other locations and the metric distance between this location and the key location
    distances = dict()
